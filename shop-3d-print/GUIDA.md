...
Crea un repository (es. `shop-3d-print`) su [github.com](https://github.com) e carica tutti i file **direttamente nella radice del repository**.

⚠️ **Importante**: quando carichi/trascini i file, assicurati che `index.html`, `api/`, `data/` ecc. compaiano subito aprendo la pagina principale del repository — NON dentro un'ulteriore sottocartella (es. `shop-3d-print/shop-3d-print/...`). Se finiscono in una sottocartella, Vercel non li trova e il sito darà errore 404. Se capita, si corregge comunque impostando "Root Directory" su Vercel (Settings → General) con il nome di quella sottocartella.
...
Nella cartella `images/`, aggiungi le foto dei tuoi pezzi stampati (formato .jpg o .png, meglio se quadrate, es. 800x800px). Il nome del file deve corrispondere a quello scritto nel campo `"immagine"` di ogni prodotto in `data/products.json`.
...
├── package.json
├── .env.example
├── index.html                 ← Home (presentazione attività)
├── prodotti.html               ← Catalogo + carrello
├── contatti.html                ← Contatti (email, Instagram, WhatsApp)
├── success.html                ← pagina dopo pagamento riuscito
├── cancel.html                  ← pagina se il cliente annulla
├── style.css
├── app.js                       ← logica carrello + checkout (usata in prodotti.html)
├── nav.js                        ← aggiorna il numero nel carrello della barra di navigazione
...
Nota: il progetto **non usa più `vercel.json`** — Vercel riconosce automaticamente la cartella `api/` come funzioni serverless e serve il resto come sito statico, senza bisogno di configurazione manuale. Questo evita anche i problemi di 404 legati a configurazioni di routing conflittuali.

## Personalizzare i testi e i contatti

- **index.html**: cambia il testo dentro `<p class="lead">` con la descrizione della tua attività, e il titolo `<title>`/logo nella navbar (`Le Mie Stampe 3D`)
- **contatti.html**: sostituisci `tuoprofilo` (Instagram), `tuaemail@esempio.com` e il numero WhatsApp (`XXXXXXXXXXX`, formato internazionale senza + o spazi, es. `393331234567`) con i tuoi dati reali
